...
class DualAttentionNetwork(nn.Module):
    def __init__(self, config):
        ...
        self.init_embed_dim=config.init_embed_dim
        ...
        self.W_o = torch.nn.Linear(config.fea_j_input_dim, self.init_embed_dim)
        self.W_m = torch.nn.Linear(config.fea_m_input_dim, self.init_embed_dim)
        self.W_e = torch.nn.Linear(config.fea_m_input_dim, self.init_embed_dim)
        self.W_b = torch.nn.Linear(config.bit_dim, self.init_embed_dim)

        for i in range(self.num_dan_layers):
            self.op_attention_blocks.append(
                MultiHeadOpAttnBlock(
                    input_dim=self.init_embed_dim,
        ...
        for i in range(self.num_dan_layers):
            self.mch_attention_blocks.append(
                MultiHeadMchAttnBlock(
                    node_input_dim=self.init_embed_dim,
                    edge_input_dim=self.init_embed_dim,
    ...
    def forward(self, fea_j, op_mask, candidate, fea_m, mch_mask, comp_idx, bit_vector):
        """
        ...
        num_ope = fea_j.size(1)

        bit_embed = self.W_b(bit_vector)
        bit_embed_op = bit_embed.unsqueeze(1).expand(sz_b,num_ope,bit_embed.size(-1))
        bit_embed_mc = bit_embed.unsqueeze(1).expand(sz_b,M,bit_embed.size(-1))

        fea_j = self.W_o(fea_j)+bit_embed_op
        fea_m = self.W_m(fea_m)+bit_embed_mc
...
class DANIEL(nn.Module):
    def __init__(self, config):
        ...
        self.pair_input_dim = config.pair_input_dim
    ...
    def forward(self, fea_j, op_mask, candidate, fea_m, mch_mask, comp_idx, dynamic_pair_mask, fea_pairs, bit_vector):
        """
